...
const MetaPairVector StrategyManager::getZergBuildOrderGoal() const
{
  ...
  if (Config::Strategy::StrategyName == "Zerg_ZerglingRush") {
    ...
    */
    ...
    if (self->minerals() > 1000 * num_mains) {
      // IT'S MACRO HATCH TIME!
      goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Zerg_Hatchery, num_hatcheries + 1));
    }
    if (num_hatcheries >= 3) {
      // THIS IS DANGEROUS
      // UNTIL CHURCHILL FIXES SOME THINGS
      goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Zerg_Lair, num_hatcheries + 1));
    }
    ...
    const int max_sunkens = 25;
    const int max_spores = 5;
    ...
    - Previous number of drones + 1
    */
    int num_desired_drones = std::min(num_mains * 15, num_drones + 1);
    goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Zerg_Drone, num_desired_drones));
